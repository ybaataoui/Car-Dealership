...
function FeaturedCarsSearch() {
    ...
    useEffect(() => {
        fetchCars();
        fetchMakes();
    }, [sortOrder]); //re-apply the sorting when the sortOrder chnage.
    ...
    const fetchCars = async () => {
        try {
            ...
            const featuredCars = response.data.filter((car) => car.is_featured == true)
            const sortedCars = sortCars(featuredCars); //sort cars
            setCars(sortedCars);
        } catch (error) {
...
export default FeaturedCarsSearch;
